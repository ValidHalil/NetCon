...
class CTkListbox(ctk.CTkScrollableFrame):
    def __init__(
        ...
        self._scrollbar2.grid_configure(padx=(5, border_width + 4))
        self._scrollbar2.configure(height=12)
        ...
        if not font:
            self.font = ctk.CTkFont(ctk.ThemeManager.theme["CTkFont"]["family"], 13)
        else:
        ...
        self._scrollbar2.configure(width=width)
        self.width = width
    ...
    def insert_many(self, index, options_list, update=True):
        """
        Вставляет список опций (options_list) за один проход,
        отключив перерисовку до самого конца.
        """

        if not options_list:
            return

        # 1. Определяем начальный индекс
        # Если index == "end", мы просто продолжаем нумерацию с конца.
        if str(index).lower() == "end":
            start_index = self.end_num
        else:
            # Если указан конкретный индекс, нам нужно сместить все существующие кнопки.
            # Это сложнее и медленнее, чем просто вставка в конец.
            print("Вставка в середину пакетом требует сложного регрейда, используйте 'end'!")
            return

        current_index = start_index

        # 2. Цикл вставки с отключением обновления
        for option in options_list:
            # Мы вызываем оригинальный insert, но передаем update=False
            # и явно указываем числовой индекс, а не "END"

            # ВНИМАНИЕ: Поскольку оригинальный insert манипулирует self.end_num
            # если мы передаем числовой индекс, нам нужно быть аккуратным.
            # Проще всего использовать числовой индекс и вручную управлять self.end_num

            # --- Адаптация к внутренней логике ---

            # Мы имитируем вызов insert, но для числовых индексов

            # Создание кнопки
            self.buttons[current_index] = ctk.CTkButton(
                self,
                text=option,
            ...
            self.buttons[current_index].configure(command=lambda num=current_index: self.select(num))

            # Размещение кнопки в сетке
            self.buttons[current_index].grid(padx=0, pady=(0, 5), sticky="nsew", column=0, row=current_index)

            if self.multiple:
                self.buttons[current_index].bind(
                    "<Shift-1>", lambda e, idx=current_index: self.select_multiple(self.buttons[idx])
                )

            current_index += 1

        # Обновляем счетчик общего количества элементов
        self.end_num = current_index

        # 3. Финальное обновление UI (Один раз)
        if update:
            self.update()
    ...
    def configure(self, **kwargs):
        ...
        if "width" in kwargs:
            self._scrollbar2.configure(width=kwargs["width"])
    ...
    def move_up(self, index):
        ...
        if index > 0:
            ...
            previous_key = list(self.buttons.keys())[0]
