...
class Loading(ctk.CTkToplevel):
    def __call__(self, ipa, textbox, type, master, opacity, language, alert_mode):
        ...
        def ping(ip,textbox,type_var):
            ...
            if self.language == "Русский":
                if self.alert_mode:
                    CTkMessagebox(opacity = opacity, message='Операция завершена!', title='Успех!', icon='check', master=self.master_window, button_width=self.master_window.alert_button_size)
                return
            else:
                if self.alert_mode:
                    CTkMessagebox(opacity=opacity, message='Operation completed!', title='Success!', icon='check', master=self.master_window, button_width=self.master_window.alert_button_size)
                return
    ...
    def get(self):
        ...
        kostil = 1
        self.after(100, lambda: self.master_window.bind('<Escape>', lambda close: self.master_window.close_app()))
        return kostil