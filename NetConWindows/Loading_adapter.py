...
class Loading_adapter(ctk.CTkToplevel):
    def __call__(self, cmd, textbox, combo, flag, master, opacity, language, alert_mode):
        ...
        def adapter(name,textbox):
            try:
                ...
                if self.language == "Русский":
                    if self.alert_mode:
                        CTkMessagebox(opacity = opacity, message='Готово! Смотрите "Console log."', title='Успех!', icon='check', master=self.master_window, button_width=self.master_window.alert_button_size)
                    return
                else:
                    if self.alert_mode:
                        CTkMessagebox(opacity=opacity, message='Done! See the "Console log".', title='Success!', icon='check', master=self.master_window, button_width=self.master_window.alert_button_size)
                    return
            except:
                combo.focus_set()
                close_window()
