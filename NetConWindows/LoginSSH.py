...
class LoginSSH(ctk.CTkToplevel):
    def __call__(self, var, master, opacity, language):
        ...
        def clicked(user_login,ip, alg):
            if user_login == "":
                if self.language == "Русский":
                    close_window()
                    CTkMessagebox(opacity = opacity, message='Поле не должно быть пустым!', title='Ошибка!', icon='cancel', master=self.master_window, button_width=self.master_window.alert_button_size)
                    return
                else:
                    close_window()
                    CTkMessagebox(opacity=opacity, message='The field must not be empty!', title='Error!', icon='cancel', master=self.master_window, button_width=self.master_window.alert_button_size)
                    return
            close_window()
            if alg == "Auto":
        ...
        self.button_1 = ctk.CTkButtonSoftHover(master=self.frame_1, font=ctk.CTkFont(family="Trebuchet MS", size=14, weight="bold"), text="ОК", width=100, normal_color="#1F538D", fg_color="#1F538D", hover=False, border_width=0, command=lambda: clicked(self.entry1.get(),ip_address, self.combobox.get()))
