...
class NetConTerminal(ctk.CTkToplevel):
        ...
        try:
            while self.running and self.net_connect:
                ...
                if self.check_esr_flag:
                    if ((b'[81D' in line.encode() and b'[80D' in line.encode() and b'10C' in line.encode()) and self.con_type == "Telnet") or ((b'[512D' and line.encode() and b'[12C' in line.encode() and b'[511D' in line.encode()) and self.con_type != "Telnet"):  # [74D # новое: детект есров
                        self.is_esr = True
                    else:
                        self.is_esr = False
                    self.check_esr_flag = False
                ...
                if "more" in line.lower():  # Other
                    if self.language != "Русский":
                        if self.help_mode and not self.is_esr:  # новое: замена для вывода справки
                        ...
                        if self.help_mode and not self.is_esr:
                ...
                elif ">" in line and len(line) < 40 and "> " not in line.rstrip() and " >" not in line and "cr" not in line.lower():
                    index = line.find(">")
                    self.find_device_name(line[:index + 1])
        except Exception as e:
            self.disconnect()
    ...
    def remove_more_and_surrounding(self, string):
        if self.con_type == "Telnet" and self.is_esr:
            pattern = r".*More.*rest."
        else:
            pattern = r".*More.*\n"
    ...
    def send_command(self, aft_con=False):  # для элтексов вроде заебца
        if self.net_connect:
            self.tab_flag = False
            #if not aft_con:
            #    self.back_text_area_read_input()  # новое: для обхода отправки комманды после справки (имитация ошибки как на табе)
            if self.help_mode and not self.esr_show_mode:
                ...
                self.help_mode = False
                self.input_entry.configure(state="normal")
                #self.update_command_list(self.input_before_help)
                self.after(100, lambda: self.input_entry.insert("end", self.input_before_help.lstrip()))
    ...
    def send_help(self, command): # новое
            ...
            if command == "?":
                ...
                self.empty_help = False
                self.input_before_help = command.replace("?","").replace("\x03 ","").replace("\r","")
            self.help_mode = True
